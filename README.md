...
Releases follow [Semantic Versioning](https://semver.org/); every release publishes a `vX.Y.Z` git tag plus a corresponding [GitHub Release](https://github.com/worldbank/data360-mcp/releases). To guarantee reproducibility, pin an **exact** version (patch updates can still change behavior, and minor releases add features); major/minor-only pinning does not lock behavior.

Install the Python server from PyPI and pin a known-good release:

```bash
pip install data360-mcp==X.Y.Z
```
...
The `@data360/*` npm UI packages are published on release; pin each to its desired version:

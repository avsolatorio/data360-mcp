...
**Outbound resilience.** All Data360 calls use the shared resilience transport built in `http_client.py`, exposed via two `httpx.AsyncClient` instances (interactive + background/bulk) with separate connection pools. It uses split timeouts (`DATA360_CONNECT_TIMEOUT`, `DATA360_READ_TIMEOUT`, `DATA360_WRITE_TIMEOUT`, `DATA360_POOL_TIMEOUT`) instead of a single wall, and retries transient failures (`retry_status_codes`, i.e. 429/502/503/504) with exponential backoff and jitter, bounded by `DATA360_RETRY_BUDGET_SECONDS`. Retries are opt-in for non-idempotent methods: GET/HEAD are always retried, while POST is retried only when the caller marks a read-only query with the `data360_retry_safe` request extension (search, metadata, and dimensions do; the Charts API write does not).

Two traffic profiles keep bulk work from starving user requests. The interactive client serves tool calls, health probes and chart writes with the tight timeouts above. A second background/bulk client (`get_background_httpx_client`) serves the ~1.7 MB dimension catalog, the dataset catalogue sync and the group-hierarchy fetch, with its own connection pool (`DATA360_BACKGROUND_*` timeouts and retry budget) so a slow bulk fetch cannot occupy the interactive pool. The catalog is loaded **without blocking a request**: `_ensure_extdataportal_loaded` waits at most `DATA360_BACKGROUND_INLINE_WAIT_SECONDS` (default 2 s) and then continues with raw dimension codes while a single shared fetch finishes in the background — request paths must tolerate missing labels, and `get_label` already falls back to the raw code.

**Metadata degradation.** `get_metadata` keeps a last-known-good snapshot per cache key. When a metadata call fails transiently (timeout, connection error, 5xx) and no fresh value can be produced, the last successful snapshot is returned with `stale: true` and the failure reason in `error`; `get_data` treats that as non-fatal and continues. The degraded outcome is then reused for `DATA360_DEGRADATION_COOLDOWN_SECONDS` so a burst of callers hits the failing dependency once per window instead of once per request. Non-transient failures (404, validation, parse) are never masked by stale data.
